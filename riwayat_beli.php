...
<nav class="navbar navbar-expand-sm navbar-dark text-monospace" style="background-color: #007291;"> 
	...
	<?php
		...
		if (!isset($_SESSION['user'])) {
			...
			$data_table = '';
			$data = select_data_orders();
			foreach ($data as $key => $val) {
				$data_table .= '
					<tr>
						<td><center><a href="detail_trans.php?id_barang='.$val['id'].'">'.$val['id'].'</center></td>
						<td><center>'.$val['name'].'<center></td>
						<td><center>'.$val['datecreation'].'</center></td>
						<td><center>'.$val['status'].'</center></td>
						<td><center>'.$val['user'].'<center></td>
					</tr>
				';
			}
			...
			echo '
			<form method="post">
				<table class="table table-bordered table-hover text-monospace" id="tabel">
					<tr class="header text-light" style="background-color: #007291;" scope="col">
						<th width="15%" nowrap><center>Kode Transaksi</center></th>
						<th nowrap  width="15%"><center>Jenis transaksi</center></th>
						<th nowrap><center>Tanggal Transaksi</center></th>
						<th width="15%" nowrap><center>Status</center></th>
